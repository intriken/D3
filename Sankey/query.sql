...
create table test.vlb_sankey_dvs
  STORED AS ORC tblproperties ("orc.compress" = "SNAPPY","auto.purge"="true") as
select event_date, traffic_source, l2_cat_description, sum(instances) instances, sum(cpv) cpv, sum(bbc) bbc
...
join deal_info dl on sf.deal_uuid = dl.deal_id
group by event_date, traffic_source, l2_cat_description;

select traffic_source, l2_cat_description, sum(cpv) cpvs, sum(INSTANCEs) from test.vlb_sankey_dvs group by traffic_source, l2_cat_description;



create table test.vlb_sankey_orders
  STORED AS ORC tblproperties ("orc.compress" = "SNAPPY","auto.purge"="true") as
select l2_cat_description, order_date, count(distinct fgt.order_id) purchases
...
and fgt.brand = 'test'
group by order_date, l2_cat_description;
